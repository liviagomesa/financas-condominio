...
- [X] T021 [P] [US1] Create `frontend/src/app/shared/models/recurring-charge.model.ts`: interfaces `RecurringCharge` (`id`, `type`, `amount`, `dueDay`, `description`, `fund: Fund`, `party: Party`, `observations`, `lastGenerationFailed`), `RecurringChargeRequest`, `RecurringChargeBulkRequest` (mesmo formato de `account.model.ts`) — **reaberta na Phase 10 (sessão 2026-08-05, T057)**: `fund: Fund` → `fund: FundSummary`
...
## Phase 10: `FundSummaryResponse` para embutimento (impacto cruzado com a feature 004, sessão 2026-08-05)

**Purpose**: A feature 004 revisou (ver `specs/004-fund-entity-balance/tasks.md`, Phase 9) o padrão de `AccountResponse.fund`, que passou de `FundResponse` completo para `FundSummaryResponse` — `recurring-charge-list.html` nunca exibiu `fund.realBalance`, e o valor era recalculado a cada linha da listagem sem uso real. `RecurringChargeResponse.fund` reaproveitava exatamente o mesmo padrão (T017/T019/T021) e recebe a mesma correção.

- [X] T055 Update `RecurringChargeResponse`: campo `fund` (`FundResponse`) → `fund` (`FundSummaryResponse`); `from(RecurringCharge recurringCharge, FundSummaryResponse fundResponse)` em vez de `from(RecurringCharge recurringCharge, FundResponse fundResponse)` em `backend/src/main/java/com/financas/recurringcharge/api/RecurringChargeResponse.java` (depends on `specs/004-fund-entity-balance/tasks.md` T040)
- [X] T056 Update `RecurringChargeController`: remove a dependência de `FundService` (não é mais necessária); `toResponse` monta `FundSummaryResponse.from(recurringCharge.getFund())` diretamente, sem consulta adicional (associação `@ManyToOne(optional = false)` já `EAGER`) em `backend/src/main/java/com/financas/recurringcharge/api/RecurringChargeController.java` (depends on T055)
- [X] T057 [P] Update `frontend/src/app/shared/models/recurring-charge.model.ts`: `RecurringCharge.fund: Fund` → `RecurringCharge.fund: FundSummary` (import de `FundSummary` em vez de `Fund`, mesma origem de `specs/004-fund-entity-balance/tasks.md` T043) (depends on `specs/004-fund-entity-balance/tasks.md` T043)

**Checkpoint**: `GET /api/recurring-charges` não recalcula mais o saldo real do fundo por linha da listagem.
