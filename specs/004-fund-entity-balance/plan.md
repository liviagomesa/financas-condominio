...
A usuária questionou, ao revisar os controllers do backend, por que `AccountResponse.fund` (e, via feature 009, `RecurringChargeResponse.fund`) embutia o `FundResponse` completo — incluindo `realBalance`, campo computado por `FundService.calculateRealBalance(Fund)` a partir de uma consulta própria (`accountRepository.findByFundId`) mais agregação em memória — quando nenhuma das duas listagens (`account-list.html`, `recurring-charge-list.html`) exibe esse valor. Como a decisão original (research.md, "`FundResponse` com valor computado") tinha rejeitado explicitamente uma segunda DTO por presumir, sem verificação, que "não há caso de uso real que precise do `Fund` sem o saldo real", essa premissa foi revisada à luz do uso real do frontend e considerada equivocada.

Mudança: novo `FundSummaryResponse` (`id`, `name`, `initialBalance`), com `from(Fund)` puro (sem parâmetro computado), passa a ser o tipo de `AccountResponse.fund`/`RecurringChargeResponse.fund`. `FundResponse` (com `realBalance`) permanece inalterado, exclusivo de `GET /api/funds`/`GET /api/funds/{id}`. `AccountController`/`RecurringChargeController` deixam de injetar `FundService` — a associação `Account.fund`/`RecurringCharge.fund` já é `@ManyToOne(optional = false)` (fetch `EAGER`), então `FundSummaryResponse.from(account.getFund())` não exige nenhuma consulta adicional (elimina, como efeito colateral, o recálculo redundante de `realBalance` uma vez por linha de listagem em vez de uma vez por fundo distinto). Ver research.md para o racional completo, incluindo a alternativa de deduplicar o cálculo por requisição (rejeitada).

Re-confirmação da Constitution Check, princípio por princípio, à luz dessa mudança:
- **I. Arquitetura em Camadas**: PASS, sem alteração necessária. `FundSummaryResponse` vive no mesmo pacote `com.financas.fund.api` de `FundResponse`, sem novo pacote/camada.
- **II. Separação Controller → Service → Repository**: PASS — na verdade simplifica: `AccountController`/`RecurringChargeController` deixam de depender de `FundService` para montar a resposta, já que `FundSummaryResponse.from` não precisa de nenhum valor calculado pelo `Service`.
- **III. Stack Técnica Definida**: PASS, sem alteração necessária — nenhuma dependência nova; testes existentes de `AccountResponse`/`AccountController` (e equivalentes de `RecurringCharge`, feature 009) são ajustados para o novo tipo embutido, sem exigir caso de teste novo (não é regra de negócio nova, é uma mudança de forma de DTO).
- **IV. Convenções de Código e Formatação**: PASS, sem alteração necessária.
- **V. Idioma por Tipo de Conteúdo**: PASS, sem alteração necessária.
- **VI. Convenções de API REST**: PASS — `FundSummaryResponse` continua respeitando "nunca só o identificador" (carrega `id`, `name`, `initialBalance`, dados reais da entidade, não uma FK solta); o campo computado (`realBalance`) permanece exclusivo do endpoint que representa o próprio recurso (`GET /api/funds`), consistente com o padrão já adotado pelo Princípio VI de campo computado exposto via `Service` — só que agora escopado ao endpoint que efetivamente precisa dele, em vez de propagado a todo embutimento.
