...
- [ ] T040 [P] Create `FundSummaryResponse` (record: `id`, `name`, `initialBalance`; factory estático `from(Fund fund)`, sem parâmetro computado) em `backend/src/main/java/com/financas/fund/api/FundSummaryResponse.java`
- [ ] T041 Update `AccountResponse`: campo `fund` (`FundResponse`) → `fund` (`FundSummaryResponse`); `from(Account account, FundSummaryResponse fundResponse)` em vez de `from(Account account, FundResponse fundResponse)` em `backend/src/main/java/com/financas/account/api/AccountResponse.java` — **reabre T016** (depends on T040)
- [ ] T042 Update `AccountController`: remove a dependência de `FundService` (não é mais necessária para montar a resposta); `toResponse` monta `FundSummaryResponse.from(account.getFund())` diretamente, sem consulta adicional (associação `@ManyToOne(optional = false)` já `EAGER`) em `backend/src/main/java/com/financas/account/api/AccountController.java` — **reabre T018** (depends on T041)
- [ ] T043 [P] Update `frontend/src/app/shared/models/fund.model.ts`: add interface `FundSummary` (`id`, `name`, `initialBalance`, sem `realBalance`); `Fund` (com `realBalance`) permanece inalterado, usado só pelo retorno de `GET /api/funds`
- [ ] T044 Update `frontend/src/app/shared/models/account.model.ts`: `Account.fund: Fund` → `Account.fund: FundSummary` (import de `FundSummary` em vez de `Fund`) — **reabre T023** (depends on T043)
- [ ] T045 [P] Update `frontend/src/app/shared/bulk-duplicate.spec.ts`: remove `realBalance` do fixture de `fund` usado no mock de `Account`, alinhando ao novo formato `FundSummary`
