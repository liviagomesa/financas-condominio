...
- [X] T016 [P] Update `AccountResponse`: campo `fund` (`Fund`) → `fund` (`FundResponse`); `from(Account account, FundResponse fundResponse)` em vez de `from(Account account)` puro em `backend/src/main/java/com/financas/account/api/AccountResponse.java` (depends on T013, T008) — **reaberta na Phase 9 (sessão 2026-08-05, T041)**: `FundResponse` → `FundSummaryResponse`
...
- [X] T018 Update `AccountController`: injeta `FundService`; monta o `FundResponse` embutido a cada `AccountResponse.from(account, FundResponse.from(account.getFund(), fundService.calculateRealBalance(account.getFund())))`, e passa `request.fundId()` em vez de `request.fund()` para `AccountService` em `backend/src/main/java/com/financas/account/api/AccountController.java` (depends on T014, T015, T016, T017) — **reaberta na Phase 9 (sessão 2026-08-05, T042)**: dependência de `FundService` removida, `FundSummaryResponse` montado direto
...
- [X] T023 [P] Update `frontend/src/app/shared/models/account.model.ts`: remove o tipo `Fund` local e `FUND_LABELS` (passam a vir de `fund.model.ts`); `Account.fund` passa a ser o objeto `Fund` embutido (importado de `./fund.model`); `AccountRequest`/`AccountBulkRequest.fund` → `fundId: number` (depends on T021) — **reaberta na Phase 9 (sessão 2026-08-05, T044)**: `Account.fund` passa a ser `FundSummary`, não mais `Fund`
...
## Phase 9: `FundSummaryResponse` para embutimento (revisão pós-implementação, sessão 2026-08-05)

**Purpose**: `AccountResponse.fund` deixa de embutir o `FundResponse` completo (com `realBalance`, computado por uma consulta própria a cada linha da listagem) e passa a embutir um `FundSummaryResponse` (`id`, `name`, `initialBalance`) — nem `account-list.html` nem `recurring-charge-list.html` (feature 009) exibem `fund.realBalance`, e o valor computado seguia sendo recalculado uma vez por linha, não uma vez por fundo distinto (ver plan.md/research.md, "Revisão pós-implementação"). Tarefas de 009 equivalentes ficam registradas em `specs/009-recurring-charges/tasks.md`, Phase 10.

- [X] T040 [P] Create `FundSummaryResponse` (record: `id`, `name`, `initialBalance`; factory estático `from(Fund fund)`, sem parâmetro computado) em `backend/src/main/java/com/financas/fund/api/FundSummaryResponse.java`
- [X] T041 Update `AccountResponse`: campo `fund` (`FundResponse`) → `fund` (`FundSummaryResponse`); `from(Account account, FundSummaryResponse fundResponse)` em vez de `from(Account account, FundResponse fundResponse)` em `backend/src/main/java/com/financas/account/api/AccountResponse.java` — **reabre T016** (depends on T040)
- [X] T042 Update `AccountController`: remove a dependência de `FundService` (não é mais necessária para montar a resposta); `toResponse` monta `FundSummaryResponse.from(account.getFund())` diretamente, sem consulta adicional (associação `@ManyToOne(optional = false)` já `EAGER`) em `backend/src/main/java/com/financas/account/api/AccountController.java` — **reabre T018** (depends on T041)
- [X] T043 [P] Update `frontend/src/app/shared/models/fund.model.ts`: add interface `FundSummary` (`id`, `name`, `initialBalance`, sem `realBalance`); `Fund` (com `realBalance`) permanece inalterado, usado só pelo retorno de `GET /api/funds`
- [X] T044 Update `frontend/src/app/shared/models/account.model.ts`: `Account.fund: Fund` → `Account.fund: FundSummary` (import de `FundSummary` em vez de `Fund`) — **reabre T023** (depends on T043)
- [X] T045 [P] Update `frontend/src/app/shared/bulk-duplicate.spec.ts`: remove `realBalance` do fixture de `fund` usado no mock de `Account`, alinhando ao novo formato `FundSummary`

**Checkpoint**: `GET /api/accounts` não recalcula mais o saldo real do fundo por linha da listagem; o valor computado só é exposto por `GET /api/funds`/`GET /api/funds/{id}`.
