...
- **Melhorar a aparência da caixa de seleção (checkbox) das listagens**: o checkbox de seleção múltipla das listagens (unidade/fornecedor/grupo/conta) hoje usa o estilo padrão do Bootstrap, sem nenhum tratamento visual dedicado. Daria pra melhorar a UX com algo como: aumentar o tamanho da área clicável, adicionar uma transição/efeito sutil ao marcar/desmarcar, e alinhar o visual ao restante da UI para deixar mais profissional — aplicado de forma centralizada (provavelmente via SCSS compartilhado) para refletir em todas as listagens que reaproveitam o trio `list-selection.ts`/`bulk-delete.ts`/`bulk-actions-bar` de uma vez, sem duplicar o ajuste tela a tela.