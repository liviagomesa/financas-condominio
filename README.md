...
- **Sem Spring Security por enquanto**: não há requisito de autenticação/autorização inicialmente.
...
- **Registro de pagamento como campo único (`paymentDate`), sem `paid` redundante**: um lançamento é "pago" quando `paymentDate` não é nulo; não existe um campo `paid` separado, para não correr o risco dos dois ficarem inconsistentes entre si.
...
- **Portas de desenvolvimento fora do padrão (backend `8082`, frontend `4202`, banco `5434`)**: originalmente `8080`/`4200`/`5432` (padrões do Spring Boot/Angular CLI/PostgreSQL). Trocadas por decisão explícita da usuária, que roda vários projetos Angular/Java possui outra instalação PostgreSQL na mesma máquina e queria evitar colisão recorrente com as portas-padrão de outros projetos.
- **`shared/components/row-actions/` — botões de editar/remover por linha, padronizados como ícones**: a tabela de Contas (feature 005) tinha ficado larga demais depois de ganhar as colunas Parte/Fundo e o total líquido, quebrando linha na tela. A correção trocou os botões/links de texto (Editar, Remover, Registrar pagamento, Alterar) por ícones SVG inline com `title`/`aria-label` — sem adicionar biblioteca de ícones nova (`bootstrap-icons` ou similar), só os paths SVG necessários direto no template. Isso criou uma inconsistência com `fund-list`/`party-list`/`group-list`, que continuavam usando texto — extraído `RowActions` (`editLink` + evento `remove`) para `shared/components/`, reaproveitado pelas quatro listagens, mesmo espírito do trio `list-selection`/`bulk-delete`/`bulk-actions-bar` já usado para seleção múltipla.
- **Pagamento parcial por duplicação de conta, não "ledger de pagamentos"**: ao implementar pagamento parcial (feature 008), avaliamos o modelo alternativo de manter `Account` com múltiplos `Payment` (saldo = valor − soma dos pagamentos) e descartamos por enquanto — exigiria emenda de constitution (muda a regra vigente de que "pago" deriva de `paymentDate`), migração de normalização dos dados existentes, e revisão da lógica de saldo de Fundo que já soma `Account.value`. Optamos por dividir a conta original em duas (valor pago + saldo restante, rotuladas "- parte N") por ser aditivo e reaproveitar a arquitetura e as telas já existentes (mesma duplicação da feature 007), proporcional ao porte do projeto.
...
- **Campo `recurring` removido por completo, não apenas desmarcado no split**: era um booleano que já não decidia nenhum comportamento do sistema; mantê-lo (mesmo só desmarcado nas contas resultantes de um pagamento parcial) obrigaria a lógica de split a tratá-lo como caso especial só para um dado sem uso real. Removido de ponta a ponta (formulário, persistência, contratos de API) para dar lugar a uma feature futura dedicada de recorrência real (execução agendada), desenhada do zero sem reaproveitar esse campo.
...
- **Geração automática/recorrente de lançamentos mensais**: hoje tanto o lançamento individual quanto o em lote (`POST /api/accounts/bulk`) são ações manuais disparadas pela usuária todo mês. Uma automação (job agendado) reduziria ainda mais o trabalho manual — a ser desenhada como uma feature própria, sem reaproveitar o campo `recurring` de `Account` (removido na feature 008 por não decidir nenhum comportamento), já que a geração idempotente e o histórico de reajustes exigem um modelo mais rico que um booleano simples.
